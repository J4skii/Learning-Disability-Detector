...
from utils import login_required, admin_required, counselor_required
from error_handlers import setup_logging, register_error_handlers, ErrorHandler
...
def validate_configuration():
    ...
    # Validate configuration using our security validator
    validator = SecurityValidator()
    issues = validator.validate_configuration(app.config)
    
    if issues:
        for issue in issues:
            print(f"Configuration Warning: {issue}")
        print("Please review configuration for production deployment.")
...
def view_student_profile(user_id):
    """View a specific student's profile (for counselors/admins)."""
    try:
        # Get the student to view
        ...
        from models import Result
        student_results = Result.query.filter_by(email=student.email).order_by(Result.timestamp.desc()).limit(10).all()
        
        return render_template('student_profile_view.html', student=student, results=student_results)
    except Exception as e:
        print(f"View student profile error: {e}")
        flash('An error occurred loading the student profile.')
        return redirect(url_for('counselor_dashboard'))
